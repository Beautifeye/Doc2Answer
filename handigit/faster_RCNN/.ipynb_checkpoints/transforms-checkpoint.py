...
class SwitchChannels(object):
    ...
    def __call__(self, image, target):
        if random.random() < self.prob:
            ...
            c1, c2, c3 = np.random.permutation(3)
            Image.merge('RGB', (channels[c1], channels[c2], channels[c3]))
        return image, target
    
    
class DataAugmentation(object):
    def __init__(self, augmentation_type=None, min_factor=None, max_factor=None):
